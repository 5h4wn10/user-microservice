...
public class AuthService {
    ...
    public boolean registerUser(AuthDTO authRequest) {
        ...
        user.setPassword(passwordEncoder.encode(authRequest.getPassword()));
        user.setFullName(authRequest.getFullName());
        user.setRoles(Collections.singleton(authRequest.getRole()));
        User savedUser = userRepository.save(user);

        // Anropa respektive tjänst för att skapa Patient eller Practitioner
        if (authRequest.getRole() == Role.PATIENT) {
            createPatientInPatientService(savedUser, authRequest);
        } else if (authRequest.getRole() == Role.DOCTOR || authRequest.getRole() == Role.STAFF) {
            createPractitionerInPractitionerService(savedUser, authRequest);
        }
    ...
    private void createPatientInPatientService(User user, AuthDTO authRequest) {
        // Skicka en POST-förfrågan till PatientService

        PatientDTO patientDTO = new PatientDTO();
        patientDTO.setId(user.getId());
        patientDTO.setName(user.getFullName());
        patientDTO.setAddress(authRequest.getAddress()); // Kan vara null
        patientDTO.setPersonalNumber(authRequest.getPersonalNumber()); // Kan vara null
        patientDTO.setDateOfBirth(authRequest.getDateOfBirth()); // Kan vara null
        patientDTO.setRoles(Collections.singleton(Role.PATIENT)); // Sätt roller

        System.out.println("Sending patient data to {}" + patientServiceUrl);
        try {
            webClientBuilder.build()
                    .post()
                    .uri(patientServiceUrl)
    ...
    private void createPractitionerInPractitionerService(User user, AuthDTO authRequest) {
        // Skicka en POST-förfrågan till PractitionerService
        webClientBuilder.build()
                .post()
                .uri("http://practitioner-service:8083/api/practitioners")
                .bodyValue(Map.of(
                        "userId", user.getId(),
                        "name", user.getFullName(),
                        "specialty", authRequest.getSpecialty()
                ))
                .retrieve()
                .toBodilessEntity()
                .block(); // Block används här eftersom vi inte kör asynkront
    }*/
